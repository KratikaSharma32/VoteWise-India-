...
const FALLBACK_PERFORMANCE = [
  {partyId:'bjp',party:'BJP',period:'2019-2024',overallScore:71,promises:[
    {id:'p1',promise:'Double farmer income by 2022',status:'partial',completionPct:40,note:'Income grew but did not double; PM-KISAN provides ₹6000/year'},
    {id:'p2',promise:'Build 10 crore pucca houses under PM Awaas Yojana',status:'inprogress',completionPct:72,note:'7.2 crore houses completed as of 2023'},
  ]},
  {partyId:'inc',party:'INC',period:'2019-2024',overallScore:58,promises:[
    {id:'p3',promise:'NYAY minimum income guarantee scheme',status:'notcompleted',completionPct:0,note:'Not implemented — INC not in central government'},
  ]},
  {partyId:'aap',party:'AAP',period:'2020-2024',overallScore:68,promises:[
    {id:'p4',promise:'Free electricity up to 200 units',status:'completed',completionPct:100,note:'Fully implemented in Delhi since 2019'},
  ]},
];
...
export default function Performance() {
  ...
  useEffect(() => {
    api.get('/api/performance').then(r => { setSummary(r.data.data || FALLBACK_PERFORMANCE); setLoading(false); })
      .catch(() => { setSummary(FALLBACK_PERFORMANCE); setLoading(false); });
  }, []);

  const load = async id => {
    if (sel === id) { setSel(null); setDetail(null); return; }
    const r = await api.get(`/api/performance/${id}`);
